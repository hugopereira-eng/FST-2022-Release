...
constexpr float NEW_LANDMARK_THRESHOLD = 0.02f;
constexpr float START_HEADING_DEV = (10.0f / 180.0f * M_PI);

std::default_random_engine Particle::rng;

Particle::Particle(bool delete_cones, int obs_incr, bool same_color)
    : observation_increment(obs_incr)
    , delete_cones_when_left_fov(delete_cones)
    , associate_same_color_only(same_color)
{
    processNoise << 0.2f, 0,
                    0,pow(2.0f * M_PI / 180, 2);
}


void Particle::updatePose(const Control &control, float delta) {
    // // draw new pose
    temp_pose = pose;
    pose = MotionModel::updatePose(pose, control, delta);
}
...
Landmark Particle::createLandmark(Observation ob) {
    ...
    lm.sigma = Matrix2f::Identity() * 15.0f;
    lm.updateColor(ob.color);
...
void Particle::updateStatistics(std::vector<bool> &observed_lm) {
    ...
    for (size_t i = 0; i < landmarks.size(); i++) {
        ...
        if (d_norm < 11.0 && theta > -M_PI_2 && theta < M_PI_2) {
